  ...
  function (query?, options?, callback?) {
    ...
    if (forceCountFn === true) {
      ...
      if (Object.keys(collation).length > 0) {
        countPromise = model.count(query).collation(collation).exec();
      } else {
        countPromise = model.count(query).exec();
      }
    } else {
      if (useEstimatedCount === true) {
        countPromise = model.estimatedDocumentCount().exec();
        ...
        if (Object.keys(collation).length > 0) {
          ...
          countPromise = model.countDocuments(query).exec();
        }
      }
    }
